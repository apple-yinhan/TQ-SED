...
if config.load_model:
    ...
    # best_loss = np.inf #np.min(loss_hist_)
    # hf.close()
    ...
    with tqdm(total=total_iteration) as pbar:
        for data in train_loader:
            ...
            if config.backbone == 'crnn':
                ...
                outputs = model(feat, asc, True)
                loss_1 = mse_loss_fun(outputs[0][:,:,:11], soft)
                loss_2 = mse_loss_fun(outputs[1], hard)
                loss_3 = mse_loss_fun(outputs[2], soft)
                loss_4 = mse_loss_fun(outputs[2][:,:,5:7], soft[:,:,5:7])
                loss_5 = mse_loss_fun(outputs[2][:,:,5], soft[:,:,5])

                loss = loss_1 + loss_2 + loss_3 + loss_4*2 + loss_5*2
        ...
        with tqdm(total=total_iteration) as pbar:
            for data in val_loader:
                ...
                if config.backbone == 'crnn':
                    ...
                    loss_2 = mse_loss_fun(outputs[1], hard)
                    loss_3 = mse_loss_fun(outputs[2], soft)
                    loss_4 = mse_loss_fun(outputs[2][:,:,4:8], soft[:,:,4:8])
                    loss_5 = mse_loss_fun(outputs[2][:,:,5], soft[:,:,5])
