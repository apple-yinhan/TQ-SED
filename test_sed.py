...
# -*- coding: utf-8 -*-
"""
Created on Sat Apr  1 21:28:57 2023
...
if __name__ == '__main__':
    ...
    for folders in ['fold1', 'fold2', 'fold3', 'fold4', 'fold5']:#
        ...
        if config.backbone == 'crnn':
            from models.dcase23_crnn import CRNN
        ...
        elif config.backbone == 'idc_sdme':
            from models.IDC_SDME import idc_sdme
            model = idc_sdme(classes_num=config.class_nums, cnn_filters=config.cnn_filters, rnn_hid=config.rnn_hid, _dropout_rate=config._dropout_rate, in_channels=1, scene_class=5, scene_embdim=128, device=device)

        # test audio information
        test_meta_path = f'/mnt/nfs2/hanyin/LASS4SED/baseline-codes/task4b_official/development_folds_baseline/{folders}_test.csv'
        ...
        with torch.no_grad():
            for item in tqdm(test_meta['filename']):
                ...
                audio_path = f'/mnt/nfs2/hanyin/LASS4SED/datasets/maestro_real/development_audio/{place}/{audioname}.wav'
                label_path = f'/mnt/nfs2/hanyin/LASS4SED/datasets/maestro_real/development_annotation/soft_labels_{place}/{audioname}.txt'
