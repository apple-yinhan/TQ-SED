...
class SegmentMixer(nn.Module):
    ...
    def __call__(self, waveforms, valid=False):
        ...
        for n in range(0, batch_size):
            ...
            for i in range(1, mix_num):
                ...
                rescaled_next_segment = dynamic_loudnorm(audio=next_segment, 
                                                         reference=segment, 
                                                         valid=valid,
                                                         **self.loudness_param)
            ...
            noise = dynamic_loudnorm(audio=noise, 
                                     reference=segment, 
                                     valid=valid,
                                     **self.loudness_param)
...
def dynamic_loudnorm(audio, reference, valid=False, lower_db=-10, higher_db=10): 
    rescaled_audio = rescale_to_match_energy(audio, reference)
    
    if valid:
        delta_loudness = 0  # 0 dB for validation
    else:
